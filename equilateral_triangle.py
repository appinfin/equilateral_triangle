...
def create_entry(): ## Ф-ция создаёт 9 виджетов Entry
    for i in range(1,4):
        ent= Entry(width=5)
...
def clear_label_err(): ## ф-ция удаляет виджет Label с текстом ошибки или результата
    label_err.grid_forget()
    
def check(): # ф-ция проверяет различие координат вершин (должны быть разными)
    if xyz[:3]==xyz[3:6] or xyz[:3]==xyz[6:9] or xyz[3:6]==xyz[6:9]:
        ...
        label_err['fg']= 'red'
        label_err.grid(row= 6, column= 0, columnspan= 4, pady=5, sticky='w')
    else:
        calculate()

def err_init(err): ## ф-ция указывает неверно введённую координату вершины
...
def get_coordinates(event): ## ф-ция получает массив координат вершин попорядку
    clear_label_err()
    global xyz
    xyz=[]
    for i in range(0,9):
...
def calculate(): ## ф-ция расчёта длин сторон треугольника и вывода результата
...
lable.grid(row=0, column=0, columnspan=4, padx=10)
