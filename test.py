...
def create_entry():
    ...
    for i in range(1,4):
        ent= Entry(width=5, textvariable= None)
...
def clear_label_err():
##    label_err= Label(font='Arial 10 bold', fg='red')
...
def check():
    if xyz[:3]==xyz[3:6] or xyz[:3]==xyz[6:9] or xyz[3:6]==xyz[6:9]:
        print("Вы ввели вершины с одинаковыми координатами")
...
##    label_err= Label(text=f"!!! Ошибка ввода !!! {dict_err[err]}",\
